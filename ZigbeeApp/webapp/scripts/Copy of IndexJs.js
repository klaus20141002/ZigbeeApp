...
$(document).ready(function () {
    ...
    window.onbeforeunload = function () //author: meizz     
    {
        ...
        if (b && window.event.clientY < 0 || window.event.altKey) {
            window.event.returnValue = "确定要退出-城市无线智能照明监控系统？";
        }
    }
    ...
    if (getCookie("UserName") != null && getCookie("Role") != null) {
        BindEqpList("treeDiv1", "灯具实时监控", "Admin/RealTimeControl.aspx");
        if (getCookie("OrgID") == "12") //江苏芯光能源管理有限公司
        {
            $("#tabs_1").hide();
            changemenu(3, "回路监控");
    ...
    //    } else {
    //        $("#tit").html("城市无线智能照明监控系统");
    //        document.title = "城市无线智能照明监控系统";
    //    }
    if (getCookie("OrgID") != "12") //江苏芯光能源管理有限公司
    {
        setTimeout("AddTab('灯具实时监控', 'Admin/RealTimeControl.aspx');", 10);
    }
...
function SelNode(ul) {
    if (ul == 22) {
        ...
        return 'Admin/RealTimeControl.aspx';
    } else if (ul == 56) {
...
function tabCloseEven() {
    ...
    $('#mm-tabupdate').click(function () {
        ...
        if (url != undefined && currTab.panel('options').title != '灯具实时监控') {
            $('#tabs').tabs('update', {
                tab: currTab,
                options: {
                    content: createFrame(url)
...
function changemenu(checkid, mCheckName) {
    ...
    switch (checkid) {
        case 1:
            $("#treeDiv1").show();
            BindEqpList("treeDiv1", "灯具实时监控", "Admin/RealTimeControl.aspx");
            AddTab('灯具实时监控', 'Admin/RealTimeControl.aspx');
            break;
        case 2:
            $("#treeDiv2").show();
            BindEqpList("treeDiv2", "地理信息", "Admin/BaiDuiMap.aspx");
            AddTab('地理信息', 'Admin/BaiDuiMap.aspx');
            break;
        case 3:
            $("#treeDiv3").show();
            BindEqpList("treeDiv3", "回路监控", "Admin/LoopControl.aspx");
            AddTab('回路监控', 'Admin/LoopControl.aspx');
            break;
        case 4:
            $("#treeDiv4").show();
            if (mBind4 == 0) {

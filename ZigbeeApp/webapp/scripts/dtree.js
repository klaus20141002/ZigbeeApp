...
function checkPar(chkBox) {
	if (chkBox.name.toLowerCase() == "checkbox" && chkBox.className != "d-1") {//判断本单击为选中,并且不是根目录,则选中父目录
	if (chkBox.checked) {
	var chkObject = document.getElementById("c" + chkBox.className);// 得到父目录对象
	chkObject.checked = true;
	checkPar(chkObject);
	} else {
		// 如果没有被选中，判断同级的子是否是全没有选中，如果选没有选中，则要将父节点置为不选中状态
		var b = false;
		$(":checkbox[class='" + chkBox.className + "']").each(function(i) {
		...
		if (!b) {
			var chkObject = document
					.getElementById("c" + chkBox.className);// 得到父目录对象
			if(chkObject!=null){
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.useIcons) {
		...
		if (this.config.check == true) {
			str += '<input type="checkbox" name="checkbox" class="'+this.obj +node.pid+'" value="' + node.id
					+ '" id="c' + this.obj + node.id + '" onclick="javascript:'
					+ this.obj + '.cc(' + node.id + ','+node.pid+')"/>';
		}
	}
